...
public class SetIntakePivotPosition extends Command {
  ...
  public SetIntakePivotPosition(Intake i, Elevator pay, double p) {
    m_intake = i;
    m_elevator = pay;
    position = p;
  ...
  public boolean isFinished() {
    if (position >= Constants.IntakeConstants.maxPivotL1Angle && (m_elevator.getPosition()) <= Constants.ElevatorConstants.CoralStationPosition - Constants.ElevatorConstants.elevatorThreshold){
      System.out.println("Cannot pivot back, Elevator is too low!");
      return true;
    }
    // System.out.println(Math.abs(m_payload.getPosition() - position));
    System.out.println("Set Intake: " + Math.abs(m_elevator.getPosition() - position));
