...
StructArrayPublisher<Pose2d> arrayPublisher;
  ...
  public PoseEstimator() {
    ...
    photonPoseEstimatorBack = visionSubsystem.getPhotonPoseEstimator();
...
arrayPublisher = NetworkTableInstance.getDefault()
  ...
  public void updateEachPoseEstimator(PhotonPoseEstimator poseEstimator, int camID){    
    if(visionSubsystem.getFrontApriltagCamera().getLatestResult().hasTargets()) {
     poseEstimator.update(visionSubsystem.getFrontApriltagCamera().getLatestResult()).ifPresent(estimatedRobotPose -> {
  ...
  public void periodic() {
    ...
    //updates the drivePoseEstimator with Navx angle and current wheel positions
  ...
  public void setCurrentPose(Pose2d newPose) {
    drivePoseEstimator.resetPosition(driveSubsystem.getHeading(), driveSubsystem.getPositions(), newPose);
  }
